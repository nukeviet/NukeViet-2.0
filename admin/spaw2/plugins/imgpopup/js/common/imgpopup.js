...
SpawPGimgpopup.imagePopupClickCallback = function(editor, result, tbi, sender)
{
  if (result)
  {
    ...
    if (!a)
    {
      ...
      a.href = "#";
      a.setAttribute('onclick', "window.open('"+editor.getConfigValue("PG_IMGPOPUP_DIALOG")+editor.getConfigValue("PG_IMGPOPUP_PARAMETER")+"="+result+"','Image','width=500,height=300,scrollbars=no,toolbar=no,location=no,status=no,resizable=no,screenX=120,screenY=100');return false;");
      // new link
