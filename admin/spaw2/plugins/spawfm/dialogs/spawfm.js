...
SpawFm.txtThumbnail = 'Enter the new size-width:';
  ...
  if (SpawFm.selectedItem != itemno) {
    ...
    if (SpawFm.items[itemno].type == 'F' && SpawFm.allowModify) {
      ...
      if(ext == 'gif' || ext == 'jpg' || ext == 'png' || ext == 'jpeg') {
      	document.spawfm_form.thumbnail_button.disabled = false;
    	 }
...
SpawFm.unselectItem = function() {
  if (SpawFm.selectedItem >= 0) {
    ...
    document.spawfm_form.thumbnail_button.disabled = true;
...
SpawFm.thumbnailClick = function() {
  var newsize = prompt(SpawFm.txtThumbnail,'');
  var check = /^([0-9])+$/;
  if (newsize && check.test(newsize)) {
    document.spawfm_form.size_width_to.value = newsize;
    document.spawfm_form.thumbnail_file.value = SpawFm.items[SpawFm.selectedItem]['name'];
    document.spawfm_form.submit();
  }
}
